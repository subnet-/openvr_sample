	...
	std::vector<unsigned int>& indices)
{
	if (suffix(filename) == "ply")
		...
		return load_obj(filename, positions, normals, texcoords, indices);
}
...
bool save_geom(const std::string& filename,
	...
	const std::vector<uvec3>& colors,
	const std::vector<unsigned int>& indices)
{
	if (suffix(filename) == "ply")
		return save_ply(filename, positions, normals, texcoords, colors, indices);
	else
		return save_obj(filename, positions, normals, texcoords, indices);
}
	...
	for (int i = 0; i < nelems; i++) {
		if (std::strcmp(elist[i], "vertex") == 0) {
			...
			for (int i = 0; i < num_elems; i++) {
				...
				if (have_colors)
					colors[i] = uvec3(ply_vertex.r, ply_vertex.g, ply_vertex.b);
			}
		}
	...
	const std::vector<unsigned int>& indices)
{
	...
	for (size_t i = 0; i < positions.size(); i++) {
		...
		if (colors.size() == positions.size()) {
			memcpy(&(element[offset]), &colors[i], sizeof(uvec3));
			offset += sizeof(uvec3);
		}
	...
	std::vector<unsigned int>& indices)
{
	...
	std::string errmsg;
	tinyobj::LoadObj(shapes, materials, errmsg, filename.c_str());
	if (!errmsg.empty()) {
		fprintf(stderr, "%s\n", errmsg.c_str());
		return false;
	}
